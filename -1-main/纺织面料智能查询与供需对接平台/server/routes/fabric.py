...
def list_fabrics():
    ...
    if keyword:
        ...
        query = query.filter(
            db.or_(
                Fabric.name.like(kw),
                Fabric.composition.like(kw),
                Fabric.craft.like(kw),
                Fabric.color.like(kw),
            )
        )
...
@fabric_bp.route('/<int:fabric_id>/favorite', methods=['GET'])
@jwt_required()
def check_favorite(fabric_id):
    """Check if current user has favorited a fabric.

    Returns:
        200: {'is_favorited': bool}
    """
    user_id = int(get_jwt_identity())
    existing = Favorite.query.filter_by(user_id=user_id, fabric_id=fabric_id).first()
    return jsonify({'is_favorited': existing is not None}), 200
